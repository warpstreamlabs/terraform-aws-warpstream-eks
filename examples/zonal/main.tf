...
module "warpstream-a" {
  ...
  resource_prefix      = "${local.name}-a"
  control_plane_region = local.region
  kubernetes_namespace = "default"

  # Setting to one since we only want to deploy into a single zone
  zone_count = 1
  ...
  # Adding additional helm sets to specify the zone selector
  additional_helm_sets = [
    {
      name  = "availabilityZoneSelector.enabled"
      value = true
      type  = "auto"
    },
    {
      name  = "availabilityZoneSelector.zone"
      value = "us-east-1a"
    },
  ]
}
